...
using MAT, Printf, Statistics, LinearAlgebra, Plots  # ATTENTION: plotting fails inside plotting library if using flag '--math-mode=fast'.
###########################################
...
@views function Stokes2D_vep()
    ...
    do_gif    = false
    do_save   = false
    runame    = "Stokes2Dvep_egc_corr_v1"
    # Physics
    Lx, Ly    =  14.1e3, 10.4e3
    pconf     =  250e6
    η0        =  1e50
    ...
    pconf     =  pconf/τc
    # Numerics
    nx        =  1*3*16 - 2 # -2 due to overlength of array nx+2
    ny        =  1*2*16 - 2 # -2 due to overlength of array ny+2
    nt        =  40
    iterMax   =  3e5
    nout      =  500
    Vdmp      =  4.0
    ...
    for it = 1:nt
        ...
        while err > ε_nl && iter < iterMax
            ...
            @parallel compute_K_W_new!(Kxz, Kyz, Wxyv, Wz, Vxe, Vye, _dx, _dy) # cosserat
            ...
            # ---- Plasticity
            @parallel check_yield_1_new!(Tii, Fc, τxx, τyy, τzz, τxy, Pt, Sin_ϕ, Cos_ϕ, Eiip_x, Eiip_y, Cc0, Mxzc, Myzc, Rxyc, c_grad, l_coss, _dx, _dy, coss)
            if (mod(iter, nout)==0)  max_Fc = maximum(Fc);  @printf("Init  Fc = %1.3e \n", max_Fc)  end
            @parallel compute_Pl_λ_dQ_new!(Plc, λcP, λc, Fc, dQdτxx, dQdτyy, dQdτzz, dQdτxy, dQdMxz, dQdMyz, dQdRxy, Tii, τxx, τyy, τzz, τxy, Mxzc, Myzc, Rxyc, λc0, η_vec, Sin_ϕ, Sin_ψ, Cos_ϕ, Hc, η_vp, K, dt, rel, l_coss)
            # Check with physical (non-relaxed) λ and Fc
            if (mod(iter, nout)==0) # ACHTUNG: for REAL Physics yield check: Fc=FcP, λc=λcP
                @parallel correct_new!(τxx, τyy, τzz, τxy, η_vec, Mxzc, Myzc, Rxyc, Pt1, Eii, dQdτxx, dQdτyy, dQdτzz, dQdτxy, dQdMxz, dQdMyz, dQdRxy, λcP, εxx1, εyy1, εzz1, εxy1, Sin_ψ, Pt, Cc, Cc0, Hc, Kxzc1, Kyzc1, Wxyv1, l_coss, l_coss2, K, dt, coss)
                @parallel check_yield_2_new!(Tii, FcP, τxx, τyy, τzz, τxy, Pt1, λcP, Sin_ϕ, Cos_ϕ, Eiip_x, Eiip_y, Cc, Mxzc, Myzc, Rxyc, l_coss, η_vp, c_grad, _dx, _dy, coss) # cosserat
                max_FcP = maximum(FcP);  @printf("Check Fc_phys = %1.3e \n", max_FcP)
            end
            # Compute with relaxed λ and Fc
            @parallel correct_new!(τxx, τyy, τzz, τxy, η_vec, Mxzc, Myzc, Rxyc, Pt1, Eii, dQdτxx, dQdτyy, dQdτzz, dQdτxy, dQdMxz, dQdMyz, dQdRxy, λc, εxx1, εyy1, εzz1, εxy1, Sin_ψ, Pt, Cc, Cc0, Hc, Kxzc1, Kyzc1, Wxyv1, l_coss, l_coss2, K, dt, coss)
            @parallel check_yield_2_new!(Tii, FcP, τxx, τyy, τzz, τxy, Pt1, λc, Sin_ϕ, Cos_ϕ, Eiip_x, Eiip_y, Cc, Mxzc, Myzc, Rxyc, l_coss, η_vp, c_grad, _dx, _dy, coss) # cosserat
            # ---- Plasticity
            ...
            if mod(iter, nout)==0
                ...
                err = maximum([norm_Rx, norm_Ry, norm_RPt, 0*norm_RW]);
            ...
            @parallel update_VP!(Vxe, Vye, Pt, Wz, dVxdτ, dVydτ, RPt, 0*dWzdτ, dτVx, dτVy, dτPt, dτWz)
            iter+=1; iterT+=1
        end
        ...
        while err > ε_nl && iter < iterMax
            ...
            if mod(iter, nout)==0
                ...
                norm_RPt = norm(RPt)/length(RPt)
                norm_RW  = norm(RW)/length(RW)
                err = maximum([0*norm_Rx, 0*norm_Ry, 0*norm_RPt, norm_RW]);
                push!(err_evo1,maximum([norm_Rx, norm_Ry, norm_RPt, norm_RW])); push!(err_evo2, iter)
                @printf("> it %d, iter %d, err = %1.3e [norm_Rx=%1.3e, norm_Ry=%1.3e, norm_RPt=%1.3e, norm_RW=%1.3e] \n\n", it, niter, err, norm_Rx, norm_Ry, norm_RPt, norm_RW)
            	if norm_Rx>1.0
            		println("oups..."); file = matopen("J_Rx_$(it)_NaN$(iter).mat", "w"); write(file, "J_Rx", Array(Rx)); close(file)
            ...
            @parallel compute_dτ!(dτVx, dτVy, dτPt, dτWz, η_vepc, η_vepv, scPt, scV, scW, min_dxy2, max_nxy) # cosserat
            @parallel compute_dV!(dVxdτ, dVydτ, dWzdτ, Rx, Ry, RW, dampX, dampY, dampW) # cosserat
            @parallel update_VP!(Vxe, Vye, Pt, Wz, 0*dVxdτ, 0*dVydτ, 0*RPt, dWzdτ, dτVx, dτVy, dτPt, dτWz)
            iter+=1; iterT+=1
        end
        ...
        if do_save
            ...
            write(file, "Tii",  Array(Tii*τc));
            write(file, "Eiit", Array(Eiitot));
            write(file, "Eiip", Array(Eiip));
            write(file, "iter", iter);
            close(file)
...
@parallel_indices (ix,iy) function initial_1!(η_ev::Data.Array, Vxe::Data.Array, Vye::Data.Array, Sin_ψ::Data.Array, Sin_ϕ::Data.Array, Cos_ϕ::Data.Array, xc, yc, xv, yv, Lx::Data.Number, Ly::Data.Number, rad::Data.Number, ε_bg::Data.Number, sin_ψ_inc::Data.Number, sin_ϕ_inc::Data.Number, cos_ϕ_inc::Data.Number)
    ...
    if (ix<=size(η_ev,1) && iy<=size(η_ev,2)) if (radv2<rad*rad)  η_ev[ix,iy] = η_ev[ix,iy]/2.0  end  end

    if (ix<=size(Sin_ψ,1) && iy<=size(Sin_ψ,2)) if (radc2<rad*rad)  Sin_ψ[ix,iy] = sin_ψ_inc  end  end
    if (ix<=size(Sin_ϕ,1) && iy<=size(Sin_ϕ,2)) if (radc2<rad*rad)  Sin_ϕ[ix,iy] = sin_ϕ_inc  end  end
    if (ix<=size(Cos_ϕ,1) && iy<=size(Cos_ϕ,2)) if (radc2<rad*rad)  Cos_ϕ[ix,iy] = cos_ϕ_inc  end  end
...
@parallel function compute_K_W_new!(Kxz::Data.Array, Kyz::Data.Array, Wxyv::Data.Array, Wz::Data.Array, Vxe::Data.Array, Vye::Data.Array, _dx::Data.Number, _dy::Data.Number)
    @all(Kxz)  = _dx*@d_xa(Wz)
    @all(Kyz)  = _dy*@d_ya(Wz)
    @all(Wxyv) = 0.5*(_dy*@d_ya(Vxe) - _dx*@d_xa(Vye))
    return
end
...
@parallel function check_yield_1_new!(Tii::Data.Array, Fc::Data.Array, τxx::Data.Array, τyy::Data.Array, τzz::Data.Array, τxy::Data.Array, Pt::Data.Array, Sin_ϕ::Data.Array, Cos_ϕ::Data.Array, Eiip_x::Data.Array, Eiip_y::Data.Array, Cc0::Data.Array, Mxzc::Data.Array, Myzc::Data.Array, Rxyc::Data.Array, c_grad::Data.Number, l_coss::Data.Number, _dx::Data.Number, _dy::Data.Number, coss::Data.Number)
    # Rheology
    @all(Tii) = sqrt( 0.5*(@all(τxx)*@all(τxx) + @all(τyy)*@all(τyy) + @all(τzz)*@all(τzz) + 2.0*@all(τxy)*@all(τxy) + coss*(@all(Mxzc)/l_coss)*(@all(Mxzc)/l_coss) + coss*(@all(Myzc)/l_coss)*(@all(Myzc)/l_coss) + 2.0*coss*(@all(Rxyc)*@all(Rxyc)) ) )
    # Check yield
    @all(Fc) = @all(Tii) - @all(Pt)*@all(Sin_ϕ) - @all(Cc0)*@all(Cos_ϕ) + c_grad*(_dx*_dx*@d2_xa(Eiip_x) + _dy*_dy*@d2_ya(Eiip_y))
    return
end

@parallel_indices (ix,iy) function compute_Pl_λ_dQ_new!(Plc::Data.Array, λcP::Data.Array, λc::Data.Array, Fc::Data.Array, dQdτxx::Data.Array, dQdτyy::Data.Array, dQdτzz::Data.Array, dQdτxy::Data.Array, dQdMxz::Data.Array, dQdMyz::Data.Array, dQdRxy::Data.Array, Tii::Data.Array, τxx::Data.Array, τyy::Data.Array, τzz::Data.Array, τxy::Data.Array, Mxzc::Data.Array, Myzc::Data.Array, Rxyc::Data.Array, λc0::Data.Array, η_vec::Data.Array, Sin_ϕ::Data.Array, Sin_ψ::Data.Array, Cos_ϕ::Data.Array, Hc::Data.Array, η_vp::Data.Number, K::Data.Number, dt::Data.Number, rel::Data.Number, l_coss::Data.Number)
    # Plastic nodes
    ...
    if (ix<=size(dQdRxy,1) && iy<=size(dQdRxy,2))  dQdRxy[ix,iy] = 1.0/Tii[ix,iy]*Rxyc[ix,iy]  end
    return
end

@parallel function correct_new!(τxx::Data.Array, τyy::Data.Array, τzz::Data.Array, τxy::Data.Array, η_vec::Data.Array, Mxzc::Data.Array, Myzc::Data.Array, Rxyc::Data.Array, Pt1::Data.Array, Eii::Data.Array, dQdτxx::Data.Array, dQdτyy::Data.Array, dQdτzz::Data.Array, dQdτxy::Data.Array, dQdMxz::Data.Array, dQdMyz::Data.Array, dQdRxy::Data.Array, λc::Data.Array, εxx1::Data.Array, εyy1::Data.Array, εzz1::Data.Array, εxy1::Data.Array, Sin_ψ::Data.Array, Pt::Data.Array, Cc::Data.Array, Cc0::Data.Array, Hc::Data.Array, Kxzc1::Data.Array, Kyzc1::Data.Array, Wxyv1::Data.Array, l_coss::Data.Number, l_coss2::Data.Number, K::Data.Number, dt::Data.Number, coss::Data.Number)
    @all(τxx)  = 2.0*@all(η_vec)*(@all(εxx1) -     @all(λc)*@all(dQdτxx))
    @all(τyy)  = 2.0*@all(η_vec)*(@all(εyy1) -     @all(λc)*@all(dQdτyy))
    @all(τzz)  = 2.0*@all(η_vec)*(@all(εzz1) -     @all(λc)*@all(dQdτzz))
    @all(τxy)  = 2.0*@all(η_vec)*(@all(εxy1) - 0.5*@all(λc)*@all(dQdτxy))
    ...
    @all(Cc)   = @all(Cc0) + dt*sqrt(2.0/3.0)*@all(λc)*@all(Hc)
    @all(Pt1)  = @all(Pt) + K*dt*@all(λc)*@all(Sin_ψ)
    @all(Eii)  = sqrt( 0.5*( @all(εxx1)*@all(εxx1) + @all(εyy1)*@all(εyy1) + @all(εzz1)*@all(εzz1) + coss*(@all(Kxzc1)*l_coss)*(@all(Kxzc1)*l_coss) + coss*(@all(Kyzc1)*l_coss)*(@all(Kyzc1)*l_coss) + 2.0*@all(εxy1)*@all(εxy1)+ 2.0*@av(Wxyv1)*@av(Wxyv1) ) )
    return
end
# ACHTUNG: for yield check: Fc=Fc_real, and λ=λ_phys !!!
@parallel function check_yield_2_new!(Tii::Data.Array, Fc::Data.Array, τxx::Data.Array, τyy::Data.Array, τzz::Data.Array, τxy::Data.Array, Pt1::Data.Array, λc::Data.Array, Sin_ϕ::Data.Array, Cos_ϕ::Data.Array, Eiip_x::Data.Array, Eiip_y::Data.Array, Cc::Data.Array, Mxzc::Data.Array, Myzc::Data.Array, Rxyc::Data.Array, l_coss::Data.Number, η_vp::Data.Number, c_grad::Data.Number, _dx::Data.Number, _dy::Data.Number, coss::Data.Number)
    # Rheology
    @all(Tii) = sqrt( 0.5*(@all(τxx)*@all(τxx) + @all(τyy)*@all(τyy) + @all(τzz)*@all(τzz) + 2.0*@all(τxy)*@all(τxy) + coss*(@all(Mxzc)/l_coss)*(@all(Mxzc)/l_coss) + coss*(@all(Myzc)/l_coss)*(@all(Myzc)/l_coss) + 2.0*coss*(@all(Rxyc)*@all(Rxyc)) ) )
    # Check yield
